...
parser.add_argument('--max_epoch', type=int, default=250, help='Epoch to run [default: 2000]')
...
print(data_batches_1.shape)
print(label_batches_1.shape)
...
train_data_index_1 = np.arange(0,data_batches.shape[0],3)
train_data_index_2 = np.arange(1,data_batches.shape[0],3)
test_data_index = np.arange(2,data_batches.shape[0],3)
train_data_index = np.concatenate((train_data_index_1,train_data_index_2),axis=0)
print(train_data_index.shape)
print(test_data_index.shape)

# 划分测试集与训练集
train_data = data_batches[train_data_index, ...]
train_label = np_class_map(label_batches[train_data_index])
test_data = data_batches[test_data_index, ...]
test_label = np_class_map(label_batches[test_data_index])
...
# 减小测试集和训练集 用于测试
#train_data = data_batches[0:2]
#train_label = np_class_map(label_batches[0:2])
#test_data = data_batches[2:4]
#test_label = np_class_map(label_batches[2:4])
#print(train_data.shape, train_label.shape)
#print(test_data.shape, test_label.shape)
#print(test_data.shape, test_label.shape)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for t in range(17):
        if total_seen_class[t] != 0:
            ...
            log_string('correct_num %d' % total_correct_class[t])
            log_string('total_seen_num %d' % total_seen_class[t])
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for t in range(17):
        if total_seen_class[t] != 0:
            ...
            log_string('correct_num %d' % total_correct_class[t])
            log_string('total_seen_num %d' % total_seen_class[t])
            log_string('acc: %f' % (total_correct_class[t] / float(total_seen_class[t])))
