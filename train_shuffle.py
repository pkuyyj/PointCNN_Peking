...
# 可视化一帧的结果
output_visualize_list = [0 for _ in range(NUM_POINT)]
    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch == MAX_EPOCH-1 or epoch == 0:
                eval_one_epoch(sess, ops, test_writer, 1)
            else:
                eval_one_epoch(sess, ops, test_writer, 0)
...
def eval_one_epoch(sess, ops, test_writer, have_output):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    total_correct = 0
    total_seen = 0
    ...
    for batch_idx in range(num_batches):
        ...
        for i in range(start_idx, end_idx):
            if have_output >= 1:
                have_output -= 1
                output_visualize_list = current_data[start_idx, :, :]
                output_correct = [0 for _ in range(NUM_POINT)]
                for j in range(NUM_POINT):
                    if pred_val[i - start_idx, j] == current_label[i,j]:
                        output_correct[j] = 1
                    else: 
                        output_correct[j] = 0
                np.save(file="output_visualize_point.npy", arr=output_visualize_list)
                np.save(file="output_visualize_correct.npy", arr=output_correct)
            else:
                for j in range(NUM_POINT):
                    l = current_label[i, j]
                    total_seen_class[l] += 1
